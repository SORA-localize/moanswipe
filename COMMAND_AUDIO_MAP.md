...
| `input.scroll.sustained` | 実入力 | scroll session 継続 0.9 秒到達時 | `scrollSustained` | `scroll_flow_01.wav` |
| `input.scroll.intense` | 実入力 | scroll session 継続 1.8 秒到達時 | `scrollIntense` | `scroll_intense_01.wav` |
...
| 内部カテゴリ | 用途 | 発火条件 | 現在のファイル数 | 実ファイル |
|---|---|---|---|---|
| `click` | 単発リアクション | menubar テスト再生、通常 click | 3 | `click_light_01.wav`, `click_soft_01.wav`, `click_sharp_01.wav` |
| `scrollShort` | 短い scroll 開始音 | 新規 scroll session 開始時 | 1 | `scroll_soft_01.wav` |
| `scrollSustained` | 中継続 scroll 音 | session 継続 0.9 秒到達時 | 1 | `scroll_flow_01.wav` |
| `scrollIntense` | 長継続 scroll 音 | session 継続 1.8 秒到達時 | 1 | `scroll_intense_01.wav` |
...
| `scrollWheel` | [`InputMonitor.swift`](/Users/hori/Desktop/MoanSwipe/MoanSwipe/InputMonitor.swift) | `ScrollSessionTracker` へ渡し、session 段階に応じて `scrollShort / scrollSustained / scrollIntense` を再生 |
