...
- scroll session による `short / sustained / intense` 分類が入っている
- 長時間 scroll 向けの pulse 実装が入り、継続中の無音化を減らす試験を進めている
- [`COMMAND_AUDIO_MAP.md`](/Users/hori/Desktop/MoanSwipe/COMMAND_AUDIO_MAP.md) でコマンドと音声の対応を管理している
- `ContentView` は本体機能ではなく補助的な位置づけである

ただし、UX 上の重要課題が残っている。

- 長時間 scroll の pulse 間隔やランダム感はまだ調整途中
- scroll カテゴリごとの通常音数が不足している
- `Volume` を含む設定管理は未着手である

したがって、次の優先事項は通常音源の拡充、scroll UX の詰め、設定管理の実装である。
...
- [x] Phase 1: menubar シェル化
- [x] Phase 2: 単発音再生基盤
- [x] Phase 3: click / tap 系入力接続
- [x] Phase 4: scroll 入力と頻度制御
- [x] Phase 5: 音声プールとランダム化
- [x] Phase 6: 低遅延・多重再生 audio 基盤
- [x] Phase 7: scroll セッション化と段階変化
...
6. Phase 6 を単独で完了させる
7. Phase 7 を単独で完了させる
8. Phase 7.1 を単独で完了させる
9. Phase 8 を単独で完了させる
10. Phase 9 を判断フェーズとして実施する
11. Phase 10 は全期間を通じて更新し、節目ごとに計画とレビュー運用を整理する
...
- 2026-04-09 v2.5: Phase 7 / 7.1 の進行を反映。進捗チェックボックス、Phase 7.1 の依存順序、現状認識、対応表整合を更新
